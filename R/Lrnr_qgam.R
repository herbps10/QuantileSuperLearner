...
Lrnr_qgam <- R6Class(
  classname = "Lrnr_qgam", inherit = Lrnr_base,
  ...
  private = list(
    ...
    .train = function(task) {
      ...
      if (is.null(args$formula)) {
        ...
        args$form <- lapply(args$formula, as.formula)
      } else {
        args$form <- as.formula(args$formula)
      }
      
      # fit
      if(length(args$qu) == 1) {
        fit_object <- sl3:::call_with_args(qgam::qgam, args)
      }
      else {
        fit_object <- sl3:::call_with_args(qgam::mqgam, args)
      }
      return(fit_object)
    },
    .predict = function(task) {
      # get predictions
      if(length(self$params$qu) == 1) {
        ...
        predictions <- qgam::qdo(private$.fit_object, qu = NULL, fun = predict, newdata = task$X, type = "response")
        predictions <- matrix(unlist(predictions), ncol = length(self$params$qu))
        predictions <- pack_predictions(predictions)
      }
      return(predictions)
    },
    .required_packages = c("qgam")
  )
)
